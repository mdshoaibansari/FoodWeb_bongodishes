...
import com.foodapp.bean.FoodBean;
import com.foodapp.bean.RelStoreFoodBean;
	...
	public RelStoreFoodBean getRelation(int relationId){
		...
		try {
			...
			String sql=null;
				...
				sql="select * from relationstorefood where relationid =?";
			
			
			PreparedStatement st=conn.prepareStatement(sql);
			st.setInt(1, relationId);
            ResultSet rs   = st.executeQuery();
            while(rs.next()) {
            
            	rsfb.setRelationId(rs.getInt(1));
            	rsfb.setStoreid(rs.getInt(2));
            	rsfb.setFoodid(rs.getInt(3));
            	rsfb.setPrice(rs.getInt(4));
            	rsfb.setPath(rs.getString(6));
            	rsfb.setQuantity(rs.getInt(5));
